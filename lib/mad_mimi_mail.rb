...
class MadMimiMail
  ...
  def deliver!(mail)
    ...
    when /^text\/html/
      mail_settings[:raw_html] = mail.body
    else
      mail_settings[:raw_plain_text] = mail.body
    end
      
    [:name, :from, :subject, :promotion_name, :list_name, :raw_yaml, :body_hash].inject(mail_settings) do |hash, header_name|
